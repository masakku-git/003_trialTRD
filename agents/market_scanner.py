...
def run_market_scanner() -> list[dict]:
    ...
    # ルールベースのスクリーニング
    candidates = _rule_based_screening(market_data)
...
def _rule_based_screening(market_data: list[dict]) -> list[dict]:
    """ルールベースのスクリーニング：変化率・出来高でスコアリング"""
    scored = []
    for d in market_data:
        ...
        if score > 0:
            scored.append({
                "symbol": d["symbol"],
                "score": round(score, 2),
                "reason": f"変化率{change_pct:+.1f}% 出来高比{vol_ratio:.1f}x",
            })
